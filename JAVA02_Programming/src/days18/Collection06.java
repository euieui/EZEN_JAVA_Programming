...
public class Collection06 {
	...
	public static void main(String[] args) {
		// 로또 번호를 저장하기 위해서 HadSet 클래스를 활용하는 예제
		// (중복을 허용하지 않는 특징을 활용)
		HashSet<Integer> lotto = new HashSet<>();
		while(lotto.size()<6) {
			// int temp = (int)(Math.random()*45 + 1);
			// lotto.add(temp);    ///--- 이 두줄을 밑에 한줄로 표현 가능
		...
		// Integer i 부분 : 오른쪽의 배열이나 리스트의 아이템을 하나를 저장할 변수 선언
		// lotto 부분 : 왼쪽 변수에 차례로 하나씩 전달해 줄 수 있는 아이템을 보유한 컬렉션 또는 배열
