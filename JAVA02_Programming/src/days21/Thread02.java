package days21;
// 프로그램의 수행 중 동시에 처리하고자 하는 직업이 생겼을때는 원래 갖고 있는 메인 Thread 외에
...
// 			2-1. Thread 클래스를 상속하는 방법과 같고 효과도 거의 같습니다.  ///--- 거의?

class ThreadB1 extends Thread{
	public void run() { ///--- Thread에 있는걸? 오버라이딩 
		for(int i=1  ; i<=10 ; i++)
			System.out.printf("ThreadB1 : i -> %d\n",i);
			try { sleep(300);
				// 0.3초간 실행을 멈추라는 명령
				// sleep 메서드는 Thread 클래스에 있는 static 메서드 입니다.
				// 사용할때  Thread.sleep()으로 사용해야 하지만 현재 위치는 자기 자신의 클래스 내부이기도 하므로
...
public class Thread02 {
	...
	public static void main(String[] args) {
		...
		// b1.run();  ///-- 그냥 메서드호출 쓰레드를 새로만든게 아니다!!!
		b1.start();
		b2.start();
		for (int i = 1 ; i<= 10 ; i++) {
			System.out.printf("maint : i -> %d\n", i);
