...
class Student {
	...
	public String toString() {
		...
		info += "age =" + this.age;  /// -- 반복문이 아니니까 이러면 한번만~ 좋네 이것도 
		return info;
	}
	// s1.equals(s2) -> this <- s1  obj <- s2
	public boolean equals(Object obj) {  /// --- 띵킹 private때문에? 아닌데 띵킹실패
		// ---- if(this.name == obj.name && this.age == obj.age)) 내가 생각한거 
		// 부모 클래스의 레퍼런스는 자식 인스턴스를 가졌을때, 부모가 물려준 멤버는 접근이 가능하지만
		// 자식이 새로만든 멤버는 접근이 불가능합니다.
		// 따라서 this와 obj가 비교가 되려면 obj를 Student 형으로 변환해야합니다.
		// Student target = (Student)obj;
		// 또한 전달된 obj 가 현재로선 어떤 인스턴스를 갖고 있는지 모르기 때문에,
		// instanceof 를 이용해서 형변환이 가능한지 보고 변환합니다.
		// 이상한 클래스가 왔으면 그냥 바로 return(메서드 종료)
		if(!(obj instanceof Student)) return false;  /// --- obj 가 Student 타입인가? 
		Student target = (Student)obj;
