...
// Object 가 상속한 메서드들 중 toString 과 함께 가장 많이 오버라딩 되는 메서드
public class Extends10_Object03 {
	...
	public static void main(String[] args) {
		...
		//  ---  지금보니 String이 클래스야~~ ㅋㅋㅋ 근데 new 이런거 안쓰는게 다 설정해놓은거구나
		String s1 = "Hello";
		String s2 = "Hello";
		if (s1 == s2) System.out.println("s1 변수와 s2 변수는 같습니다.");
		else System.out.println("s1 변수와 s2 변수는 다릅니다."); // --- 응? 저번에 저리 비교하면 안되지 않았다고 하지 않았나
		// s1 에는 HEAP 영역에 "Hello"를 저장한 후 그 주소가 리턴되어 저장됩니다.
		// s2 에는 이미 HEAP 영역에 "Hello" 가 저장되어 있으므로, 그주소가 저장   ----- 소름이네~ 둘다 주소가 똑같구나
		...
		// 저장하고 각각의 주소를 s3 와 s4에 저장합니다.
		// 결국 (==) 이 연산자는 글자들의 비교가 아니라. 레퍼런스 주소들의 비교라는
		// 의미 입니다.
		
		// 실제 객체의 ㄱ밧을 비교하기 위한 equals 메소드를 사용하면 참조값이 서로 다른
		...
		System.out.println(p2.hashCode());   // 둘이 완전히 다르지 new가 붙었으니 
		*/
		
		
		if (p1.equals(p2))
			System.out.println("p1 변수와 p2변수는 같습니다.(equals)");
		else 
			System.out.println("p1 변수와 p2변수는 다릅니다.(equals)");
		// equals 메서드는 Object 클래스의 메서드로서 최초는 레퍼런스값(주소)들의
		// 비교를 정의하고 있습니다.
		// String 클래스는 이미 equals 메서드를 실제값의 비교로 오버라이딩 되어 있으며
		// 기타 다른 사용자 정의 클래서에서는 별도로 equals 를 실제값으로 비교하도록
		// 오버라이딩 해줘야 합니다.
		
		/// --- 오버라이딩이 안되어있어서 그럼 ! 
		/// --- s3, s4 는 즉, String 클래스는 equals가 오버라이딩 되어있다
	}
}
