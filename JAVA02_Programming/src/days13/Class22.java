...
// 인스턴스 메서드에서는 스태틱 변수를 사용할 수 있습니다.
// 스태틱 메서드는 인스턴스 변수를 사용할 수 없습니다.
// 스태틱 메서드는 스태틱 변수를 사용할 수 있습니다.

class MemberCall{
	// 인스턴스 변수 : 객체가 생성될때 iv 변수가 생성 -> 10으로 초기화  ------ 아까 내 띵킹이 틀린 이유가 이거다!!!!!!
	int iv = 10; 
	...
	// iv가 언제 생성될지 모름, sv2에 값을 전달할 수 없음
	static int sv2 = new MemberCall().iv;  // ---객체.iv 를 만든다 , but 객체 주소를 모름 잃어버리는 객체  
	// 임시방편으로 객체 생성후 변수를 사용하는 예
	// 억지로라도 인스턴스(멤버)변수로 스태틱 변수의 값을 초기화하고자 한다면
	// 위와 같은 방법을 씁니다. 레퍼런스 변수 없는 new 인스턴스를 만들어서 멤버변수 사용
	
	// 인스턴스 메서드는 인스턴스 변수, 스태틱 변수, 인스턴ㅌ스 메서드, 스태틱 메서드
	// 모두 자유롭게 사용 가능
