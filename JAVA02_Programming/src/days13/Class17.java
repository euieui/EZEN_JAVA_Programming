package days13;
// static 키워드
// 클래스 (중첩클래스에서만 사용 가능) , 멤버 변수 , 멤버메소드에 적용할 수 있는 키워드
...
public class Class17 {
	...
	public static void main(String[] args) {
		...
		a.num = 100;
		System.out.printf("a 객체의 동적 멤버변수 값 : %d\n",a.num);
		// 위에서 객체 생성명령(StaticA a = new StaticA();) 이 없다면
		// 아래 두개의 명령은 모두 에러가 발생합니다.
		
		// 그러나 스태틱 변수는 main 메서드 실행전에, 객체생성 전에
		// 이미 생성되어 있는 객체와는 독릭접, 클래스와는 종속적으로 사용하도록 생성 됩니다.
		// 객체를 생성하지 않고도 사용할 수 있는 클래스의 멤버 변수
		
		// 동적변수는 객체 생성마다 그 객체 안에 생성되서 객체의 갯수만큼 생성되지만
		// 정적변수는 프로그램 전체를 통틀어 한개 만들어집니다.
		// 그 값도 일관성있게 끝까지 유지됩니다.
		
		// 다만 클래스 외부에서 static 변수에 접근하려면 위와 같이 클래스 이름과
		// 함께 사용합니다 .   
		// --내가 소름인게 저말로만 듣고 밑에 걸 썼단말야 이렇게 사용하구나?
		// --- 진짜 대단하다고 생각안드니?
		System.out.printf("%d\n",StaticA.number);
		
		// 동적 변수와 마찬가지로 private 으로 보호 되지 않았따면 임의 접근이 가능하고
		StaticA.number = 10;
		System.out.println(StaticA.number);
		//private로 보호된 static 변수는 static 메서드를 사용하여 값을 저장하거나 얻어 냅니다.
		// static 이 아닌 멤버메서들에서는 접그니 불가능합니다.
		int number=100; // --- StaticA.number 랑 달라~
