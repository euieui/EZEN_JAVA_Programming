...
public class Formatter01 {
	...
	public static void main(String[] args) {
		// 날짜 데이트 클래스, Calendar 와는 다름
		Date today = new Date(); //java.util.date
		System.out.println(today);
		
		// 날짜 서식 클래스
		SimpleDateFormat sdf1 = new SimpleDateFormat("yy-MM-dd");  ///---yyyy-MM-dd 년도 4개 쓴다 서식을 맞춰주는거네!
																								  ///--- MM은 월 m은 분 그래서 대문자 쓴다
		// 날짜 데이터를 서식 데이터에 맞춰서 적용후 출력
		System.out.println(sdf1.format(today));    ///---- format에 캘린더는 못넣나 ? 그니가 Date 클래스를 꺼낸것 같은데 ?? 
		...
		sdf4 = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss a");   ///---- 보기만해도 알겠다 소문자 hh는 a랑 같이 써주기!
		// today 적용후 출력
		System.out.println(sdf2.format(today));
		System.out.println(sdf3.format(today));
		System.out.println(sdf4.format(today));
		
		// y : 년도를 표시할 자리수 문자 (yyy:20202 yy:20)
