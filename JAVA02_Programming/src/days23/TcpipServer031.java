...
class Sender extends Thread{
	...
	Sender(Socket s ){  // main 에서 연결하고 전달해준 통신소켓을 생성자의 매개변수로 받습니다
		...
		try {
			...
			out = new DataOutputStream(socket.getOutputStream()); ///-- 위에 2줄을 한줄로 ~
			name = "[" + socket.getInetAddress() + ":" +socket.getPort() + "]";
		} catch (IOException e) {e.printStackTrace();}
	}
	public void run() {
		Scanner sc = new Scanner(System.in);
		while(out != null) {   ///--- null 뜻은 서버랑 클라이언트가 연결이 끊어졌다는 뜻
			try {
...
public class TcpipServer031 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			s = sS.accept();
			// 앞선 예제에서는 DataOutputStream 에 s를 사용하여 출력 또는 DataInputStream 에 s를 활용하여 입력
			// 그러나 본 예제는 s 를 Sender 와 Receiver 의 생성자에 전달인수로 보내서
			// 현재 연결의 입력 출력을 자유롭게 가능하게 제작합니다.
			// 각클래스는 Thread 를 상속 받아, 동시실행이 가능하게 합니다.
