...
public class MultiChatServer {
	...
	MultiChatServer(){
		...
		// 멀티 쓰레드 환경에서 단일 해쉬맵에 멀티스레드가 동시 접근하여 값을 변경하는것을 막는 동기화 도구
	}
	...
	class ServerReceiver extends Thread{
		...
		ServerReceiver(Socket s){
			...
			try {
				...
				out = new DataOutputStream(socket.getOutputStream());  ///--- 받고 보내야 하니까 Output도 필요해
				// 송수신을 위한 객체 생성(생성자)
		...
		public void run() {
			...
			try { 
				...
				name = in.readUTF();
				// 연결된 클라이언트를 해쉬맴에 저장
				clients.put(name, out); // 해쉬맵에 클라이언트 한명 추가 (키: 이름 , 벨류 : out 객체)
	...
	public void sendToAll(String msg) {
		...
		while(it.hasNext()) { // 다음 키값이 있따면 true 로 반복, 키값의 마지막까지 다다를때까지 반복실행
			
			try {
				// clients에서 (next)키값으로 얻어낸 Value(클라이언트 전용 out 객체)를 꺼내서 out에 저장
	...
	public void sijack() {
		...
		Socket s =null; // 각 클라이언트 와 통신 제어
		try {
			while(true) {   ///---- try 안에 넣은 이유가 뭐야? 연결이 끊기거나 뭐어쩌구 했는데 프로그램을 종료 해버릴려고?
				ss = new ServerSocket(9999);
				System.out.println("서버가 시작되었습니다.");
				s = ss.accept();
				System.out.println("["+s.getInetAddress()+":"+s.getPort()+"]"+"에서 접속하였습니다.");
				ServerReceiver th = new ServerReceiver(s);
				// 매 접속하는 클라이언트 마다 새로운 소켓과 쓰레드가 생성되며 각각 독립적 송숭신이 가능하게 합니다.
				th.start(); // 쓰레드 시작
			}
		} catch (IOException e) {e.printStackTrace();}
	...
	public static void main(String[] args) {
		MultiChatServer k = new MultiChatServer();   /// ---- 내느낌엔 시작에서 뭔가 잘못함!! 중복이 되는 기분이야
		k.sijack();
...
//// 인터넷(브라우저) 마다 조금씩달라서
/// 내가 웹페이즈를 만들어도 브라우저마다 다르게 나올 수 도 있음  
/// 크로스 브라우징 (표준 브라우징?)  브라우저마다 다 똑같이 보이게 만들게!
/// 웹퍼블리셔가 이런일을 한다! 브라우저마다 다똑같이 보이게 만들게! 표준화 작업
