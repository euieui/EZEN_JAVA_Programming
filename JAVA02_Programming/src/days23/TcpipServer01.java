	...
	public static void main(String[] args) {
		...
		while(true) {
			try {
				...
				Socket s = ss.accept();
				// 클라이언트의 연결요청이 오면 클라이언트 소켓과 통신할 새로운 통신용 소켓을 연결정보와 함께 생성하고,
				// 소켓이 요청받은 클라이언트 연결정보를 전달합니다.
				System.out.println(f.format(new Date()) + s.getInetAddress() + " 로부터 연결요청이 들어왔습니다."); ///--- 연결요청이 아니라 사실상 연결된것
				OutputStream out = s.getOutputStream();
				DataOutputStream dos = new DataOutputStream(out); // 최종 통신 출력 도구 : dos
				dos.writeUTF("[공지] 연결 성공 !!! 여리네 서버에 접속되셨습니다.");
				System.out.println(f.format(new Date())+ "메시지를 전송했습니다.");
				dos.close();
				s.close();
				// ss.close();  while 넣어서 지움
			} catch (IOException e) {e.printStackTrace();}
		}
